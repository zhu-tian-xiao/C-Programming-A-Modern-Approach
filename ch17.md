...
Def: dynamic storage allocation: the ability to allocate storage during program execution
...
`malloc` is the most used, it is more efficient than `calloc`.
### Null Pointers
When a memory allocation function is called, there's always a possibility that it won't be able to locate a block of memory large enough to satisfy our request. If that should happen, the function will return a null pointer.
