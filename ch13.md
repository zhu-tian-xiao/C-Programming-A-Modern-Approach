...
           "fjwoei");
```
### How Strings Literals Ares Stored
In essence, C treats string literals as character arrays.
...
```
这里的重点是strcpy和strncpy的理解
如果保证不会越界，也就是src的长度小于dest的长度，就使用strcpy，如果不能保证，就使用strncpy

strncpy**不会**自动补上0，因此需要手动添加0
...
```
和strncpy不同的是，strncat**始终**会在末尾补上0
